...
amzn_aps_csm.errors = [], amzn_aps_csm.reportErrors = function (a) {
    ...
    function c(a) {
        if (a && a.s) {
            var b, c = a.s.length > 0 ? a.s[0] : "", d = a.s.length > 1 ? a.s[1] : "";
    ...
    function d(a, d) {
        ...
        if (i = a.stack || (a.err ? a.err.stack : ""), i && i.split) for (j = i.split("\n"); o < j.length && n.s.length < g;) k = j[o++], k && n.s.unshift(b(k)); else for (l = f(a.args || arguments, "callee"), o = 0, p = 0; l && g > o;) m = h, l.skipTrace || (k = l.toString(), k && k.substr && (m = 0 === p ? 4 * h : m, m = 1 === p ? 2 * h : m, n.s.unshift(k.substr(0, m)), p++)), l = f(l, "caller"), o++;
    ...
    var g = 20, h = 256, i = /\(?([^\s]*):(\d+):\d+\)?/, j = /.*@(.*):(\d*)/;
    a.constructErrorMessage = d
}(amzn_aps_csm), window.JSON || (window.JSON = {
    parse: function (sJSON) {
        return eval("(" + sJSON + ")")
    }, stringify: function () {
        var a = Object.prototype.toString, b = Array.isArray || function (b) {
    ...
    return a.init = function () {
        ...
        var f = {elem: b, eventName: c, cb: d};
        a.eventHandlers.push(f)
    }, a.registerPostMessageHandler = function (a) {
        var b = window.addEventListener ? "addEventListener" : "attachEvent", c = window[b],
...
}), amzn_aps_csm = amzn_aps_csm || {}, function () {
    var a, b;
    amzn_aps_csm.loadModules([{name: "eventUtils", params: []}]);
    ...
    amzn_aps_csm.loadModules([{
        ...
        if (amzn_aps_csm.log("parent received message!: ", a.data), amzn_aps_csm.log(a.origin), /pixelId/.test(a.data)) {
            ...
            for (e = {}, "object" == typeof apstag && null !== apstag && "function" == typeof apstag._getSlotIdToNameMapping && null !== apstag._getSlotIdToNameMapping() && (e = apstag._getSlotIdToNameMapping()), f = "", g = document.getElementsByTagName("iframe"), h = 0; h < g.length; h++) if (g[h].contentWindow === c) {
                i = g[h].parentElement;
